...
const MANIFEST_PATH = path.join(__dirname, '../app/config/schemas/api_manifest.json');
...
function main() {
  ...
  const scanDirectory = (dir) => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (entry.name === 'node_modules' || entry.name === '.next' || entry.name === 'platform') continue;
  ...
  const auditFile = (filePath, validMethods) => {
    ...
    const importRegex = /import\s*\{([^}]+)\}\s*from\s*["']@\/app\/actions\/platform\/[^"']+["']/g;
    let match;

    while ((match = importRegex.exec(content)) !== null) {
      const importsStr = match[1];
      const imports = importsStr.split(',').map(s => s.trim().split(/\s+as\s+/)[0]); // handle aliases as well
      ...
      for (const imp of imports) {
        ...
        if (!validMethods.has(imp)) {
          ...
          console.error(`     Reason: '${imp}' is not in the platform's whitelisted manifest.`);
          failed = true;
        }
      }
    }
  ...
  if (failed) {
    ...
    console.log('  -> Integrity verification passed. All UI links are verified and safe.');
    console.log('----------------------------------------------------\n');
  }
}
