...
"""Static contract tests for erp/nextjs's gateway calls.

Run from the repository root:

    python3 -m unittest discover -s erp/nextjs/tests -v
...
surface built on it is silently empty (erp_sdk 1.0.1). Every
...
CONVERTED = {
    "templates/app/services/all/hrms/departments.ts": (
        'gatewayCall(\n        systemClient,\n        "frappe.client.get_list"',
        "getClient }",
    ),
    "templates/app/services/all/hrms/designations.ts": (
        'gatewayCall(\n        systemClient,\n        "frappe.client.get_list"',
        "getClient }",
    ),
}
...
class ReleaseContract(unittest.TestCase):
    def test_manifest_version_matches_changelog(self):
        ...
        self.assertEqual(manifest["name"], "erp_sdk")
        with open(CHANGELOG, encoding="utf-8") as fh:
            top = next(line for line in fh if line.startswith("## "))
        self.assertEqual(top.strip(), f"## {manifest['version']}")
        self.assertEqual(manifest["version"], "1.0.1")

    def test_manifest_requires_gateway_rpc(self):
        with open(MANIFEST, encoding="utf-8") as fh:
            manifest = json.load(fh)
        self.assertIn("app/lib/gateway-rpc.ts", manifest["requires"])
        self.assertIn("app/lib/gateway-rpc.ts", manifest["_comment"])
        for rel in CONVERTED:
            with open(os.path.join(SDK_ROOT, rel), encoding="utf-8") as fh:
                source = fh.read()
            self.assertIn(
                'import { gatewayCall } from "@/app/lib/gateway-rpc";', source, rel
            )
            # The control-site read must stay on the system control client.
            self.assertNotIn("paasCall(", source, rel)
            self.assertNotIn("platformCall(", source, rel)
            self.assertIn('BaseService.call("frappe.client.insert"', source, rel)
