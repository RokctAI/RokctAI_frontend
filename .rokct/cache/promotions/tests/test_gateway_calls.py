...
"""Static contract tests for promotions/nextjs: every platform call goes out
through the base kernel's gateway, never through the frappe-js-sdk client.

Run from the repository root:

    python3 -m unittest discover -s promotions/nextjs/tests -v
...
(promotions_sdk 1.1.1). The one thing that must never come back is that shape:
...
# The sites promotions_sdk 1.1.1 converted: host path -> the verbatim cmd each
# now hands to paasCall.
CONVERTED = {
    "app/actions/promotions/admin/marketing.ts": "frappe.client.get_list",
}
