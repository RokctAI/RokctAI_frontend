...
// the header's extension button. Run by tests/test_manifest.py against
...
import copy, { CHROME_WEB_STORE_MARK, GOOGLE_PLAY_MARK, LOCAL_MARKS, withLocalMarks } from './agent-hero-copy.ts';
...
describe('agent-hero-copy: what rokct.ai lays over base\'s hero', () => {
  it('names the local files for the marks, and nothing on any other host', () => {
    assert.deepEqual(CHROME_WEB_STORE_MARK, LOCAL_MARK);
    assert.deepEqual(GOOGLE_PLAY_MARK, PLAY_MARK);
    assert.deepEqual(LOCAL_MARKS, { chrome: LOCAL_MARK, 'google-play': PLAY_MARK });
    assert.ok(!JSON.stringify(LOCAL_MARKS).includes('http'));
  });

  it('gives the Chrome Web Store and Google Play badges their marks and changes nothing else', () => {
    ...
    // coloured Play mark; the App Store badge keeps base's Apple glyph.
    const play = badges.find((b) => b.id === 'google-play');
    assert.ok(play);
    assert.deepEqual(play.icon, PLAY_MARK);
    assert.equal(play.label, 'Google Play');
    assert.equal(badges.find((b) => b.id === 'app-store')?.icon, 'app-store');
