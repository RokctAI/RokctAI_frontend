...
// browser lives in the sibling ./pricing.client.tsx and is rendered
// from here.
...
import { PricingSection } from "@/components/custom/pricing.client";

export { Pricing, PricingSection } from "@/components/custom/pricing.client";
 ...
 * (before the comparison deck) and its DOM id. It is not a floating-nav
 * stop - the "Pricing" entry scrolls to copied-pricing, as in the shell.
 */
export const meta: PageSectionMeta = {
  order: 60,
  nav: [],
  anchor: "pricing-original",
};

/** The registered form: the plans the page prefetched go to the client half, which keeps the category tab state. */
export default function PricingEntry({ id, plans }: PageSectionProps) {
  return <PricingSection id={id} plans={plans} />;
}
