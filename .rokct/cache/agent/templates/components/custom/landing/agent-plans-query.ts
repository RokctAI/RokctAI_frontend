...
// platform's `Subscription Plan` catalog WITHOUT its hosting plans and
// WITHOUT its paas plans.
//
...
// platform's home SDK registers paas only, and no plan is ever named or
// numbered here.
//
...
// runs on the server: the hosting and paas rows are never fetched, so
// nothing hands them to a section; `pricing.hiddenCategories` in
// agent-landing-config.ts names "hosting" and "paas" as well, the
// client-side belt to this server-side brace. A host whose
 ...
 * hosting shell, paas to the delivery-platform storefront.
 */
const HOSTING_CATEGORY = "Hosting";
const PAAS_CATEGORY = "paas";
const EXCLUDED_CATEGORIES: readonly string[] = [HOSTING_CATEGORY, PAAS_CATEGORY];
