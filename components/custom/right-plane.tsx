...
import {
  getCalendarEvents,
  CalendarEvent,
} from "@/app/actions/handson/all/workspace/calendar";
...
function RightPlaneContent({
  ...
  const isHrManager = isDemo
    ? true
    : roles.includes("HR Manager") || roles.includes("System Manager");
      ...
      setEvents([
        {
          ...
          starts_on: "2024-01-01 10:00",
          location: "Online",
          event_type: "Meeting",
        },
        {
          subject: "Team Sync",
          starts_on: "2024-01-01 14:00",
          location: "Office",
          event_type: "Internal",
        },
      ]);
  ...
  return (
    <Sidebar
      ...
      collapsible="icon"
      className={cn(
        "!overflow-visible !absolute right-0 top-0 bottom-0 !flex-row border-l border-border/30 bg-background z-50",
      ...
      >
          ...
          <div className="border-b px-4 py-3 flex items-center justify-between h-14 shrink-0">
            ...
            {attendance && (
              <div title={`You are ${attendance.status}`}>
                <div
                  className={`h-3 w-3 rounded-full ${attendance.status === "Checked In" ? "bg-green-500 animate-pulse" : "bg-gray-300"}`}
                />
              </div>
            )}
          </div>
                  ...
                  {events.map((evt, i) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground mb-1">
                        {evt.starts_on.split(" ")[0]} •{" "}
                        {evt.starts_on.split(" ")[1]}
                      </div>
                      {evt.location && (
        ...
        <div className="flex flex-col bg-muted/10 backdrop-blur-sm w-full py-6 items-center">
          <TabsList className="flex flex-col h-auto w-full bg-transparent p-0 gap-6">
            <TabsTrigger
              value="calendar"
              className="group relative w-10 h-10 rounded-2xl data-[state=active]:bg-transparent data-[state=active]:border-2 data-[state=active]:border-primary transition-all p-0 hover:bg-muted/20"
            >
