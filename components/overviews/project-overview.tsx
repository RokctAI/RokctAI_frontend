  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-4 text-white w-full">
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <span
          className={`text-xs font-semibold px-2 py-1 rounded-full text-white ${status_color[project.status]}`}
        >
        ...
        <div className="flex -space-x-2 overflow-hidden">
          {project.users.map((user) =>
            user.avatar ? (
              <Image
                key={user.id}
                className="inline-block size-8 rounded-full ring-2 ring-zinc-900"
                src={user.avatar}
                alt={user.name}
                width={32}
                height={32}
              ...
              <div
                key={user.id}
                className="inline-flex items-center justify-center size-8 rounded-full ring-2 ring-zinc-900 bg-zinc-700"
              >
