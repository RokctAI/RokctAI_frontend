...
function SelectSeats(_a) {
    var chatId = _a.chatId, _b = _a.availability, availability = _b === void 0 ? SAMPLE : _b;
    var append = (0, react_1.useChat)({
        id: chatId,
        body: { id: chatId },
        maxSteps: 5,
    }).append;
    return (<div className="flex flex-col gap-2 bg-muted rounded-lg">
      <div className="flex flex-col gap-4 scale-75">
        ...
        {availability.seats.map(function (row, index) { return (<div key={"row-".concat(index)} className="flex flex-row gap-4">
            {row.map(function (seat, seatIndex) { return (<>
                {seatIndex === 3 ? (<div className="flex flex-row items-center justify-center w-full text-muted-foreground">
                    {index + 1}
                  </div>) : null}
                <div key={seat.seatNumber} onClick={function () {
                    append({
                        role: "user",
                        content: "I'd like to go with seat ".concat(seat.seatNumber),
                    });
                }} className={(0, classnames_1.default)("cursor-pointer group relative size-8 sm:size-10 flex-shrink-0 flex rounded-sm flex-row items-center justify-center", {
                  ...
                  <div className={(0, classnames_1.default)("absolute -top-1 h-2 w-full scale-125 rounded-sm", {
