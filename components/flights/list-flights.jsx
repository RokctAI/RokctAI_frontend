...
function ListFlights(_a) {
    var chatId = _a.chatId, _b = _a.results, results = _b === void 0 ? SAMPLE : _b;
    var append = (0, react_1.useChat)({
        id: chatId,
        body: { id: chatId },
        maxSteps: 5,
    }).append;
    return (<div className="rounded-lg bg-muted px-4 py-1.5 flex flex-col">
      {results.flights.map(function (flight) { return (<div key={flight.id} className="cursor-pointer flex flex-row border-b dark:border-zinc-700 py-2 last-of-type:border-none group" onClick={function () {
                append({
                    role: "user",
                    content: "I would like to book the ".concat(flight.airlines, " one!"),
                });
            }}>
          <div className="flex flex-col w-full gap-0.5 justify-between">
            <div className="flex flex-row gap-0.5 text-base sm:text-base font-medium group-hover:underline">
              <div className="text">
                {(0, date_fns_1.format)(new Date(flight.departure.timestamp), "h:mm a")}
              </div>
              <div className="no-skeleton">–</div>
              <div className="text">
                {(0, date_fns_1.format)(new Date(flight.arrival.timestamp), "h:mm a")}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-0.5 justify-between">
            <div className="flex flex-row gap-2">
              <div className="text-base sm:text-base">
                {(0, date_fns_1.differenceInHours)(new Date(flight.arrival.timestamp), new Date(flight.departure.timestamp))}{" "}
                hr
              </div>
            </div>
