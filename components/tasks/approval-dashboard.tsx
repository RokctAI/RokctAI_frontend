...
export function ApprovalDashboard({
  ...
  const handleAction = async (
    doctype: "Leave Application" | "Expense Claim",
    name: string,
              ...
              leaves.map((leave) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="text-xs text-zinc-400">
                        {leave.leave_type} • {leave.total_leave_days} Day(s)
                      </div>
                    </div>
                    <div className="text-xs text-zinc-500 bg-zinc-900 px-1.5 py-0.5 rounded">
                      {leave.name}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        handleAction("Leave Application", leave.name, "Reject")
                      }
                    ...
                    <button
                      onClick={() =>
                        handleAction("Leave Application", leave.name, "Approve")
                      }
                      disabled={processing === leave.name}
            ...
            expenses.map((expense) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <div className="text-xs text-zinc-400 font-mono">
                      ${expense.total_claimed_amount.toFixed(2)}
                    </div>
                  </div>
                  <div className="text-xs text-zinc-500 bg-zinc-900 px-1.5 py-0.5 rounded">
                    {expense.name}
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      handleAction("Expense Claim", expense.name, "Reject")
                    }
                  ...
                  <button
                    onClick={() =>
                      handleAction("Expense Claim", expense.name, "Approve")
                    }
                    disabled={processing === expense.name}
                    className="flex-1 bg-green-900/20 hover:bg-green-900/40 text-green-300 py-1.5 rounded text-xs transition-colors border border-green-900/30"
                  >
