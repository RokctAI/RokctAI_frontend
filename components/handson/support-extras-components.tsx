...
} from "@/app/actions/handson/all/crm/support/sla";
import {
  getWarrantyClaims,
  createWarrantyClaim,
} from "@/app/actions/handson/all/crm/support/warranty";
...
export function ServiceLevelAgreementList() {
  ...
  useEffect(() => {
    getServiceLevelAgreements().then((d) => {
      setList(d);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Service Level Agreements</h2>
        <Button
          onClick={() => router.push("/handson/all/commercial/support/sla/new")}
        >
            ...
            <TableBody>
              {list.map((l) => (
                <TableRow key={l.name}>
                  ...
                  <TableCell>{l.default_priority}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export function ServiceLevelAgreementForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (res.success) {
      ...
      router.push("/handson/all/commercial/support/sla");
  ...
  return (
    <div className="space-y-6 max-w-xl mx-auto">
      ...
      <Card>
        <CardHeader>
          <CardTitle>New SLA</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>Service Level Name</Label>
            <Input
              value={formData.service_level}
              onChange={(e) =>
                setFormData({ ...formData, service_level: e.target.value })
              }
            />
          </div>
          <div className="space-y-2">
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button
          onClick={() =>
            router.push("/handson/all/commercial/support/warranty/new")
          }
        >
...
export function WarrantyClaimForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (res.success) {
      ...
      router.push("/handson/all/commercial/support/warranty");
    } else toast.error("Failed: " + res.error);
    setLoading(false);
  };
