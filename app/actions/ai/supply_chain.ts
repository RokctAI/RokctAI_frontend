...
export async function getActiveQuotations(data: { modelId?: string } = {}) {
  ...
  if (!(await verifySupplyChainRole()))
  ...
  try {
    ...
    return {
      success: false,
      error: e?.message || "Failed to fetch quotations",
    };
  }
}
...
export async function createAiPurchaseOrder(data: {
  supplier: string;
...
export async function checkStock(data: { itemQuery: string }) {
  ...
  try {
    ...
    if (!items?.message || items.message.length === 0) {
      return {
        success: false,
        message: `No items found matching "${data.itemQuery}"`,
      };
    }
...
export async function createAiStockEntry(data: {
