...
export async function updateSmartStatus({
  query,
  status,
  document_type,
}: SmartActionInput) {
  ...
  try {
    ...
    return {
      success: false,
      message: `Action "${status}" not supported for ${type} "${doc.name}" yet.`,
    };
  } catch (e: any) {
...
async function convertQuoteToOrder(quoteName: string) {
  ...
  const { getQuotation } =
    await import("@/app/actions/handson/all/accounting/selling/quotation");
  const { createSalesOrder } =
    await import("@/app/actions/handson/all/accounting/selling/sales_order");
  ...
  if (result.success) {
    ...
    return {
      success: true,
      message: `Sales Order created from Quote ${quoteName}.`,
    };
  }
  return result;
}
...
async function convertOrderToDelivery(orderName: string) {
  ...
  const { createDeliveryNote } =
    await import("@/app/actions/handson/all/accounting/selling/delivery_note");
  ...
  if (result.success) {
    ...
    return {
      success: true,
      message: `Goods Delivered against Order ${orderName}.`,
    };
  }
  return result;
}
