...
export async function POST(request: Request) {
  const {
    ...
    messages,
    model,
  }: { id: string; messages: Array<any>; model: string } = await request.json();
  ...
  if (blockRequest) {
    return new Response("Daily Quota Exceeded. Hands-on mode enabled.", {
      status: 403,
    });
  }
  ...
  const result = await streamText({
    ...
    tools: {
      ...
      save_onboarding_progress: {
        description:
          "Saves the partial or complete Strategic Plan during onboarding.",
        parameters: z.object({
        ...
        execute: async (planData) => {
          ...
          const { saveOnboardingProgress } =
            await import("@/app/actions/ai/onboarding");
      ...
      getWeather: {
        description:
          "Get the weather forecast. If a location is not provided, uses the user's saved location.",
        parameters: z.object({
          location: z.string().optional().describe("City name (e.g. 'Musina')"),
          date: z
            .string()
            .optional()
            .describe("Specific date to check (e.g. '2025-12-16')"),
        }),
        execute: async ({ location, date }) => {
          ...
          if (!targetLocation) {
            return {
              error: "Please specify a location (e.g., 'Weather in Musina').",
            };
          }
          ...
          try {
            ...
            const baseUrl = (session.user as any).siteName
              ? (session.user as any).siteName.startsWith("http")
                ? (session.user as any).siteName
                : `https://${(session.user as any).siteName}`
            ...
            const res = await fetch(
              `${baseUrl}/api/method/${apiMethod}?location=${encodeURIComponent(targetLocation)}`,
              {
                method: "GET",
                headers: {
                  Authorization: `token ${session.user.apiKey}:${session.user.apiSecret}`,
            ...
            if (weatherData.location) {
              ...
              if (targetLocation && targetLocation.includes(",")) {
                ...
                if (parts.length > 1) {
                  const userCountryHint = parts[parts.length - 1]
                    .trim()
                    .toLowerCase(); // "za"

                  // Compare hints
                  const isCodeMatch =
                    returnedCode.toLowerCase() === userCountryHint;
                  const isNameMatch = returnedCountry
                    .toLowerCase()
                    .includes(userCountryHint);
            ...
            const adaptedData = {
              current: {
                time:
                  weatherData.location?.localtime || new Date().toISOString(),
            ...
            if (date && adaptedData.hourly.time.length > 0) {
              ...
              const noonIndex = adaptedData.hourly.time.findIndex(
                (t: string) => {
                  const d = new Date(t);
                  return (
                    d.getDate() === targetDate.getDate() && d.getHours() === 12
                  );
                },
              );
              if (noonIndex !== -1) {
                adaptedData.current.time = adaptedData.hourly.time[noonIndex];
                adaptedData.current.temperature_2m =
                  adaptedData.hourly.temperature_2m[noonIndex];
              }
            }
      ...
      learnWeatherAlias: {
        description:
          "Teach the system that a specific city name actually refers to another name (e.g., 'Musina' means 'Messina'). Use this when the user corrects a wrong location.",
        parameters: z.object({
          original: z
            .string()
            .describe("The name the user typed (e.g. 'Musina')"),
        ...
        execute: async ({ original, corrected }) => {
          ...
          try {
            ...
            const callMethod = async (method: string) => {
              return fetch(
                `${baseUrl}/api/method/${method}?original=${encodeURIComponent(original)}&corrected=${encodeURIComponent(corrected)}`,
                {
            ...
            return {
              success: true,
              message: "Thanks! I've learned this correction for next time.",
            };
          } catch (e) {
      ...
      createProject: {
        description:
          "Create a new Project. Use this when the user wants to start a new project/workspace to organize tasks.",
        parameters: z.object({
          name: z.string().describe("The name of the project"),
          description: z
            .string()
            .optional()
            .describe("A brief description of the project's goal"),
        }),
      ...
      createGoal: {
        description:
          "Create a new Performance Goal (KPI). Use this when the user wants to set a goal for themselves.",
        parameters: z.object({
        ...
        execute: async ({ description, start_date, end_date }) => {
          ...
          const result = await createAiGoal({
            description,
            start_date,
            end_date,
          });
      ...
      createTask: {
        ...
        parameters: z.object({
          ...
          project: z
            .string()
            .optional()
            .describe("The name or ID of the project to link this task to"),
        }),
      ...
      display_leave_application: {
        description:
          "Display an interactive Leave Application form for the user to pick dates.",
        parameters: z.object({
      ...
      applyLeave: {
        ...
        execute: async ({ leave_type, from_date, to_date, reason }) => {
          ...
          const result = await applyAiLeave({
            leave_type,
            from_date,
            to_date,
            reason,
          ...
          if (result.success) {
            return {
              success: true,
              message: `Leave application submitted for ${from_date} to ${to_date}.`,
            };
          } else {
            return { error: result.error };
          }
      ...
      initiate_resignation: {
        description:
          "Initiate the employee resignation process (quitting the job). Handles confirmation UI.",
        parameters: z.object({}),
        execute: async () => {
          // Just trigger the UI
      ...
      createExpense: {
        description:
          "Draft an expense claim. If user provides a receipt/image, the text content of the message will contain the URL.",
        parameters: z.object({
      ...
      update_my_profile: tool({
        description:
          "Update your professional profile with bank-level details (ID, Bank, Tax).",
        parameters: z.object({
          ...
          id_number: z
            .string()
            .optional()
            .describe("Update 13-digit SA ID Number"),
          bank_name: z.string().optional().describe("Update bank name"),
          bank_account_no: z
            .string()
            .optional()
            .describe("Update bank account number"),
          bank_branch_code: z
            .string()
            .optional()
            .describe("Update bank branch code"),
      ...
      mark_attendance: {
        description:
          "Mark attendance (Check-in/Check-out). Always triggers UI for Geolocation.",
        parameters: z.object({
          log_type: z
            .enum(["IN", "OUT"])
            .optional()
            .describe(
              "Override log type (IN/OUT). Defaults to toggling status.",
            ),
        }),
      ...
      request_asset: tool({
        ...
        parameters: z.object({
          itemName: z
            .string()
            .describe(
              "The name of the item to request (e.g. 'Macbook Pro', 'iPhone 15')",
            ),
        ...
        execute: async ({ itemName, reason }) => {
          ...
          return await createAssetRequest({
            item_name: itemName,
            reason,
      ...
      request_advance: tool({
        description:
          "Request a salary advance (Internal Staff/Employees ONLY).",
        parameters: z.object({
          amount: z.number().describe("The amount requested"),
          purpose: z
            .string()
            .describe("Reason for the advance (e.g. Medical emergency)"),
        }),
        execute: async ({ amount, purpose }) => {
          const { createAiEmployeeAdvance } =
            await import("@/app/actions/ai/hr");
          return await createAiEmployeeAdvance({
            amount,
            purpose,
            modelId: model.modelId,
          });
      ...
      getLeaveStats: {
        ...
        execute: async () => {
          ...
          const isAllowed = await verifyHrRole();
          if (!isAllowed)
            return "You are not authorized to view Leave Statistics.";
      ...
      approve_request: tool({
        description:
          "Approve a pending request (Leave, Expense, Material Request).",
        parameters: z.object({
          doctype: z
            .enum(["Leave Application", "Expense Claim", "Material Request"])
            .describe("Type of document"),
          name: z
            .string()
            .describe("ID of the document (e.g., HR-LAP-2024-001)"),
      ...
      draft_competitor: tool({
        ...
        execute: async ({ name }) => {
          ...
          if (!isAllowed)
            return "You are not authorized to manage Competitors.";
          return { name };
        },
      }),
      ...
      get_competitors: tool({
        ...
        execute: async () => {
          ...
          const { getAiCompetitors } =
            await import("@/app/actions/ai/competitor");
      ...
      analyze_competitor: tool({
        description:
          "Analyze a specific competitor's strengths and weaknesses using AI.",
        parameters: z.object({
        ...
        execute: async ({ name }) => {
          ...
          const isAllowed = await verifyCrmRole();
          if (!isAllowed)
            return "You are not authorized to analyze Competitors.";

          const { analyzeAiCompetitor } =
            await import("@/app/actions/ai/competitor");
      ...
      get_my_leads: tool({
        description:
          "Fetch a list of active CRM leads you are currently managing.",
        parameters: z.object({}),
        execute: async () => {
      ...
      create_lead: tool({
        ...
        parameters: z.object({
          lead_name: z
            .string()
            .describe("The name of the lead (First Name or Full Name)"),
          organization: z
            .string()
            .optional()
            .describe("Associated company or organization"),
      ...
      displayNote: {
        ...
        parameters: z.object({
          ...
          color: z
            .enum(["yellow", "blue", "green", "red", "purple", "pink", "gray"])
      ...
      displayProjectOverview: {
        ...
        parameters: z.object({
          ...
          status: z
            .enum(["in_progress", "on_hold", "complete", "canceled"])
          ...
          users: z
            ...
            .describe("Users assigned to the project."),
          task_count: z
            .number()
            .describe("The total number of tasks in the project."),
        }),
      ...
      disambiguate_task_type: {
        description:
          "Asks the user to clarify which type of task they want to create.",
        parameters: z.object({
          taskTitle: z
            .string()
            .describe(
              "The original title of the task the user wanted to create.",
            ),
        }),
      ...
      display_task_with_reminders: {
        description:
          "Displays a created task card along with reminder options.",
        parameters: z.object({
      ...
      set_reminder_date: {
        ...
        parameters: z.object({
          taskId: z
            .string()
            .describe("The ID of the task to set a reminder for."),
          when: z
            .enum(["Today", "Tomorrow", "Next Week"])
            .describe("When to set the reminder."),
        }),
      ...
      check_stock: tool({
        ...
        parameters: z.object({
          itemQuery: z
            .string()
            .describe("The item name or code to check stock for"),
        }),
      ...
      create_purchase_order: tool({
        ...
        execute: async ({ supplier, items }) => {
          ...
          const isAllowed = await verifySupplyChainRole();
          if (!isAllowed)
            return "You are not authorized to create Purchase Orders.";

          const { createAiPurchaseOrder } =
            await import("@/app/actions/ai/supply_chain");
      ...
      create_stock_entry: tool({
        ...
        execute: async ({ source_warehouse, target_warehouse, items }) => {
          ...
          const isAllowed = await verifySupplyChainRole();
          if (!isAllowed)
            return "You are not authorized to manage Stock Entries.";

          const { createAiStockEntry } =
            await import("@/app/actions/ai/supply_chain");
          return await createAiStockEntry({
            source_warehouse,
            target_warehouse,
            items,
          });
      ...
      smart_status_update: tool({
        description:
          "Smartly update document status or trigger next workflow steps using fuzzy logic. Use this for 'Approve quote', 'Deliver order', 'Reject invoice', etc.",
        parameters: z.object({
          query: z
            .string()
            .describe(
              "The document identifier or customer name, e.g. 'Woolworth', 'Q-2024-001'",
            ),
          status: z
            .enum(["Approved", "Rejected", "Delivered", "Paid", "Cancelled"])
            .describe("The new status or action intent"),
          document_type: z
            .enum(["Quotation", "Sales Order", "Purchase Order", "Invoice"])
            .optional()
            .describe("Explicit document type if known"),
        }),
        execute: async ({ query, status, document_type }) => {
          const { updateSmartStatus } =
            await import("@/app/actions/ai/smart_status");
          return await updateSmartStatus({ query, status, document_type });
        },
      }),
      manage_holiday_work: tool({
        description:
          "Check for upcoming holidays and allow the manager to schedule work or announcements.",
        parameters: z.object({}),
        execute: async () => {
