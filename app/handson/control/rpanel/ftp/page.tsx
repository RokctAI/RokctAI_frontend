...
import {
  ...
  createFtpAccount,
  deleteFtpAccount,
} from "@/app/actions/handson/control/rpanel/ftp/manage-ftp";
...
function FtpContent() {
  ...
  const handleCreate = async () => {
    ...
    setIsSaving(true);
    const res = await createFtpAccount(
      selectedSite,
      newUsername,
      newFtpPassword,
    );
  ...
  return (
    <>
              ...
              {filteredAccounts.map((ftp, idx) => (
                <tr key={idx} className="hover:bg-[#252b42] transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-300">
                    {ftp.quota_mb ? `${ftp.quota_mb} MB` : "Unlimited"}
                  </td>
      ...
      <Dialog
        open={!!editingFtp}
        onOpenChange={(open) => !open && setEditingFtp(null)}
      >
