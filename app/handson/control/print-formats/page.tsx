...
export default function MasterPrintFormatsPage() {
  ...
  const [editingFormat, setEditingFormat] = useState<MasterPrintFormat | null>(
    null,
  );
  ...
  const SAMPLE_PAYSLIP_DATA = {
    ...
    deductions: [{ salary_component: "Tax", amount: "500" }],
    net_pay: "6500",
  };
  ...
  useEffect(() => {
    if (
      editingFormat?.doc_type === "Appointment Letter" ||
      editingFormat?.doc_type === "Offer Letter"
    ) {
  ...
  const renderPreview = (template: string, data: any) => {
    ...
    rendered = rendered.replace(
      /\{\{\s*doc\.(\w+)\s(?:or\s*doc\.(\w+)\s)*\}\}/g,
      (match, firstVar, secondVar) => {
    ...
    const loopRegex =
      /\{%\s*for\s+(\w+)\s+in\s+doc\.(\w+)\s*%\}([\s\S]*?)\{%\s*endfor\s*%\}/g;
        ...
        <div className="flex gap-4 mb-2">
          ...
          >
            <SelectTrigger className="w-[200px]">
              <SelectValue
                placeholder={t("app.control.print_formats.ph_doctype")}
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="Sales Invoice">
                {t("app.control.print_formats.sales_invoice")}
              </SelectItem>
              <SelectItem value="Quotation">
                {t("app.control.print_formats.quotation")}
              </SelectItem>
              <SelectItem value="POS Invoice">
                {t("app.control.print_formats.pos_invoice")}
              </SelectItem>
              <SelectItem value="Offer Letter">
                {t("app.control.print_formats.offer_letter")}
              </SelectItem>
              <SelectItem value="Appointment Letter">
                {t("app.control.print_formats.appointment_letter")}
              </SelectItem>
              <SelectItem value="Salary Slip">
                {t("app.control.print_formats.salary_slip")}
              </SelectItem>
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">
            {t("app.control.print_formats.page_title")}
          </h1>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                {t("app.control.print_formats.col_doctype")}
              </TableHead>
              <TableHead className="w-[100px]">
                {t("app.control.print_formats.col_actions")}
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
