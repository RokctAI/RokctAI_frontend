...
import {
  getPlanOnAPage,
  updatePlanOnAPage,
} from "@/app/actions/handson/all/projects/strategy/plan";
import {
  getVisions,
  createVision,
  updateVision,
  deleteVision,
  getVision,
} from "@/app/actions/handson/all/projects/strategy/vision";
import {
  getPillars,
  createPillar,
  updatePillar,
  deletePillar,
} from "@/app/actions/handson/all/projects/strategy/pillar";
import {
  getStrategicObjectives,
  createStrategicObjective,
  updateStrategicObjective,
  deleteStrategicObjective,
} from "@/app/actions/handson/all/projects/strategy/objective";
import {
  getKPIs,
  createKPI,
  updateKPI,
  deleteKPI,
} from "@/app/actions/handson/all/projects/strategy/kpi";
import {
  getPersonalMasteryGoals,
  createPersonalMasteryGoal,
  updatePersonalMasteryGoal,
  deletePersonalMasteryGoal,
} from "@/app/actions/handson/all/projects/strategy/mastery";
...
export default function StrategyPage() {
  ...
  const [dialogType, setDialogType] = useState<
    "Vision" | "Pillar" | "Objective" | "KPI" | "Goal" | null
  >(null);
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const [poapData, visData, pilData, objData, kpiData, goalData] =
        await Promise.all([
          getPlanOnAPage(),
          getVisions(),
          getPillars(),
  ...
  const openDialog = (
    type: "Vision" | "Pillar" | "Objective" | "KPI" | "Goal",
    item?: any,
    parentId?: string,
  ) => {
    setDialogType(type);
  ...
  // Grouping
  const getPillarsForVision = (vid: string) =>
    pillars.filter((p) => p.vision === vid);
  const getObjsForPillar = (pid: string) =>
    objectives.filter((o) => o.pillar === pid);
  const getKpisForObj = (oid: string) =>
    kpis.filter((k) => k.strategic_objective === oid);
    ...
    <div className="space-y-8 p-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage Vision, Pillars, Objectives, and KPIs.
          </p>
        </div>
          ...
          {activeVision ? (
            <div className="space-y-8">
              ...
              <div className="text-center space-y-4 p-8 bg-primary/5 rounded-lg border border-primary/20">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: activeVision.description }}
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {getPillarsForVision(activeVision.name).map((pillar) => (
                  <Card
                    key={pillar.name}
                    className="flex flex-col h-full border-t-4 border-t-blue-500 shadow-sm"
                  >
                    <CardHeader>
                              ...
                              <div>
                                ...
                                <div className="mt-2 pl-2 border-l-2 border-muted space-y-2">
                                  {getKpisForObj(obj.name).map((kpi) => (
                                    <div
                                      key={kpi.name}
                                      className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/30 p-1.5 rounded"
                                    >
                ...
                {getPillarsForVision(activeVision.name).length === 0 && (
                  <div className="col-span-full text-center py-12 text-muted-foreground">
                    No Pillars defined for this Vision. Switch to the "Manage
                    Entities" tab to add them.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="text-center py-20 bg-muted/10 rounded-lg border border-dashed">
              ...
              <p className="text-muted-foreground mb-4">
                Select a vision from the dropdown or create a new one.
              </p>
              <Button onClick={() => openDialog("Vision")}>
                Create Vision
              </Button>
            </div>
          )}
        ...
        <TabsContent value="goals" className="space-y-6 mt-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-muted-foreground">
                Define and track individual growth objectives.
              </p>
            </div>
              ...
              <Card key={goal.name}>
                ...
                <CardContent className="flex justify-end gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => onDelete("Goal", goal.name)}
                  >
                ...
                <AccordionContent className="bg-muted/5 p-4 rounded-b-lg border-x border-b">
                  <div className="flex justify-end gap-2 mb-4">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => openDialog("Vision", vision)}
                    >
                      <Pencil className="h-3 w-3 mr-1" /> Edit Vision
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => openDialog("Pillar", null, vision.name)}
                    >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      className="text-red-500"
                      onClick={() => onDelete("Vision", vision.name)}
                    >
                  ...
                  <div className="space-y-4 pl-4 border-l-2 border-primary/20">
                    {getPillarsForVision(vision.name).map((pillar) => (
                      <div
                        key={pillar.name}
                        className="bg-background border rounded-md p-4"
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex gap-1">
                            <Button
                              ...
                              variant="ghost"
                              className="h-6 w-6"
                              onClick={() => openDialog("Pillar", pillar)}
                            >
                            ...
                            <Button
                              ...
                              onClick={() => onDelete("Pillar", pillar.name)}
                            >
                              <Trash2 className="h-3 w-3" />
                            </Button>
                            <Button
                              ...
                              onClick={() =>
                                openDialog("Objective", null, pillar.name)
                              }
                            >
                              + Objective
                            </Button>
                        ...
                        <div className="space-y-2 pl-4 border-l-2 border-blue-200 mt-2">
                          {getObjsForPillar(pillar.name).map((obj) => (
                            <div
                              key={obj.name}
                              className="bg-muted/20 rounded p-2"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                ...
                                <div className="flex gap-1">
                                  <Button
                                    ...
                                    className="h-5 w-5"
                                    onClick={() =>
                                      openDialog("Objective", obj, pillar.name)
                                    }
                                  >
                                    <Pencil className="h-3 w-3" />
                                  </Button>
                              ...
                              <div className="pl-6 mt-1 space-y-1">
                                {getKpisForObj(obj.name).map((kpi) => (
                                  <div
                                    key={kpi.name}
                                    className="flex items-center justify-between text-xs text-muted-foreground hover:bg-muted/50 rounded px-1"
                                  >
      ...
      <Dialog open={!!dialogType} onOpenChange={() => closeDialog()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingItem ? "Edit" : "Create"} {dialogType}
            </DialogTitle>
          </DialogHeader>
          <Form {...form}>
