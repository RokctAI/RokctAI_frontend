...
export default function WriteOffPage() {
  ...
  const handleWriteOff = async () => {
    if (
      !confirm(
        "CRITICAL WARNING: This will permanently write off the outstanding balance as Bad Debt. This cannot be undone. Are you sure?",
      )
    )
      return;
        ...
        <div className="p-6 bg-red-50 border-b border-red-100 flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-red-700 text-sm">
              For Loan #{loan.name} ({loan.applicant})
            </p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          <div className="bg-gray-50 p-4 rounded-xl space-y-2">
            ...
            <p className="text-xs text-gray-400 mt-2">
              Plus accrued interest (calculated on submission)
            </p>
          </div>

          <div className="space-y-4">
            <div className="p-4 bg-yellow-50 text-yellow-800 text-sm rounded-lg border border-yellow-100">
              <strong>Note:</strong> Writing off a loan will mark it as "Written
              Off" and post the loss to the Bad Debts Expense account. This
              action is generally irreversible.
            </div>
