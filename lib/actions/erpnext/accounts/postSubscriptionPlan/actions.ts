...
export async function createSubscriptionPlan(
  doc: SubscriptionPlan, apiKey: string, apiSecret: string
): Promise<SubscriptionPlan> {
  const response = await fetch(`/api/v1/resource/Subscription%20Plan`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to create subscription plan: ${errorBody}`);
  }
