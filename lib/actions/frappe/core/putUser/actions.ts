...
export async function updateUser(name: string, doc: Partial<User>, apiKey: string, apiSecret: string): Promise<User> {
  const response = await fetch(`/api/v1/resource/User/${encodeURIComponent(name)}`, {
    method: "PUT",
    headers: {
